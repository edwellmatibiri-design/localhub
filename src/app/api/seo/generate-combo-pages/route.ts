...
export async function POST(request: Request) {
  ...
  const suburb = String(body?.suburb ?? "sandton");
  ...
  const ai = await callAI(
    `Generate an SEO page for ${category} in ${suburb} with internal links.`,
  );
  const draft = ai.text;
  ...
  const topSellers = findTopRatedSellers(category, suburb);
