...
function computeSignals(
  ...
  const disputeCount = reviews.filter((row) => row.flagged).length;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data: reviews, error: reviewsError } = await supabase
      .from("reviews")
      .select("rating, created_at, flagged")
    ...
    return NextResponse.json({ ok: true, vendorId, trustScore, signals });
  } catch (error) {
    return NextResponse.json(
      {
        ok: false,
        error: "Failed to update vendor trust",
