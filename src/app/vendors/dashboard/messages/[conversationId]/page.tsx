...
export default function VendorConversationPage({ params }: Props) {
  const [conversationId, setConversationId] = useState<number>(0);
  const [conversation, setConversation] = useState<Conversation | null>(null);
  const [messages, setMessages] = useState<Message[]>([]);
  const [senderVendorId, setSenderVendorId] = useState("");
  ...
  useEffect(() => {
    ...
    async function fetchConversation() {
      try {
        ...
        if (convo?.vendor_id) {
          setSenderVendorId(String(convo.vendor_id));
        }
      } catch (err) {
        setError(
    ...
    async function detectVendor() {
      ...
      const { data: vendor } = await supabase
        ...
        .limit(1)
        .maybeSingle();
      setSenderVendorId(String(vendor?.id ?? ""));
    }
  ...
  const canSend = useMemo(
    () =>
      Boolean(draft.trim()) &&
      Boolean(senderVendorId) &&
      Boolean(conversationId),
    [draft, senderVendorId, conversationId],
  );
  ...
  return (
    <section className="shell space-y-4 p-6">
      <h1 className="text-2xl font-semibold">Vendor Conversation</h1>
      {loading && (
      ...
      <div className="card space-y-3">
        ...
        <div className="max-h-[440px] space-y-2 overflow-y-auto pr-1">
          {messages.map((item) => (
            <div
              key={item.id}
              className={`rounded-lg p-3 text-sm ${item.sender_type === "vendor" ? "bg-lh-accent/10" : "bg-lh-surface-soft"}`}
            >
